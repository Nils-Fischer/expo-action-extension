...
class ActionReactNativeDelegate: RCTDefaultReactNativeFactoryDelegate {
  ...
  override func bundleURL() -> URL? {
    ...
    if let bundleURL = settings.jsBundleURL(forBundleRoot: ".expo/.virtual-metro-entry") {
      if var components = URLComponents(url: bundleURL, resolvingAgainstBaseURL: false) {
        // Use actionExtension=true query param for metro config detection
        components.queryItems = (components.queryItems ?? []) + [URLQueryItem(name: "actionExtension", value: "true")]
...
class ActionExtensionViewController: UIViewController {
  ...
  deinit {
    print("🧹 ActionExtensionViewController deinit")
    cleanupAfterClose()
  }
  ...
  private func loadReactNativeContent() {
    getShareData { [weak self] sharedData in
      ...
      reactNativeFactoryDelegate = ActionReactNativeDelegate()
      ...
      print("[ActionExtension] self.view.contentScaleFactor before adding subview: \(self.view.contentScaleFactor)")
      print("[ActionExtension] UIScreen.main.scale: \(currentScale)")
      ...
      // Read module name from Info.plist (set by plugin per extension)
      // Defaults to "actionExtension" if not set
      let moduleName = Bundle.main.object(forInfoDictionaryKey: "ActionExtensionModuleName") as? String ?? "actionExtension"
      
      // Create the React Native root view with dynamic module name
      let reactNativeRootView = reactNativeFactory!.rootViewFactory.view(
          withModuleName: moduleName,
          initialProperties: initialProps
      )
      
      // Read background color from Info.plist (ActionExtensionBackgroundColor key)
      let backgroundFromInfoPlist = Bundle.main.object(forInfoDictionaryKey: "ActionExtensionBackgroundColor") as? [String: CGFloat]
      let heightFromInfoPlist = Bundle.main.object(forInfoDictionaryKey: "ActionExtensionHeight") as? CGFloat
  ...
  private func cleanupAfterClose() {
    ...
    print("🧹 ActionExtensionViewController cleaned up")
  }
